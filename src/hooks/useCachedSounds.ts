...
async function cacheFile(remoteUrl: string, subfolder: 'sounds' | 'images'): Promise<string> {
    ...
    try {
        ...
        await AsyncStorage.setItem(storageKey, JSON.stringify({ url: remoteUrl, localUri: uri }));
        return uri;
    } catch {
        return remoteUrl;
    }
